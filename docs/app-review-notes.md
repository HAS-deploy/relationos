...
Contacts, notes, reminders, interactions, and tags are stored only on-device (App-Group UserDefaults). RelationOS uses PostHog for anonymous product analytics (paywall views, feature use). The PostHog SDK is configured with `personProfiles=.never`, `captureScreenViews=false`, `captureApplicationLifecycleEvents=false`, `sessionReplay=false` — events leave the device tagged with a random per-install identifier only. No contact data, no IDFA, no name/email. The PrivacyInfo.xcprivacy manifest declares Product Interaction, Crash, Performance, and Other Diagnostic Data (all linked=NO, tracking=NO, purpose=Analytics + App Functionality). App Privacy nutrition answers match. The privacy policy linked from the paywall describes the same setup.
